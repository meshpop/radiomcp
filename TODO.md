...
### mpv 프로세스 종료 버그
- [x] MCP `stop()` 함수가 mpv 프로세스를 제대로 종료하지 못함 ✅ 수정됨
- [x] radio.py 수정 시 stop 로직도 함께 검토 필요 ✅ 수정됨
- 증상: stop 호출 후에도 mpv가 백그라운드에서 계속 실행됨
- 수정: terminate() 후 wait(timeout=2), 실패 시 kill() 강제 종료
