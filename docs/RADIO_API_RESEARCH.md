...
| Shoutcast | 50K+ | XML API | Requires API key (free dev key available) |
| Icecast | 15K+ | Yellow Pages | Direct listing |
| TuneIn | 100K+ | Unofficial | Scraping required |
...
def can_crawl(url: str) -> bool:
    ...
    return rp.can_fetch("RadioCliBot/1.0", url)
```

### Incremental Crawling
```python
# Don't re-crawl everything

def should_crawl(station: dict) -> bool:
    last_crawled = station.get('crawled_at')
    quality = station.get('quality_score', 50)
    ...
    # High quality = less frequent crawling
    if quality >= 80:
...
CREATE TABLE stations (
    ...
    -- Basic Info
    name TEXT NOT NULL,
    name_normalized TEXT,  -- For search (lowercase, no accents)
    description TEXT,
    ...
    -- Media
    favicon TEXT,
    favicon_valid BOOLEAN DEFAULT 0,
    ...
    genres TEXT,  -- JSON array (normalized)

    -- Technical
    codec TEXT,
    bitrate INTEGER,
    hls BOOLEAN DEFAULT 0,
    ...
    -- Source tracking
    source TEXT,  -- 'radiobrowser', 'shoutcast', 'crawler', etc.
...
-- Indexes
CREATE INDEX idx_name ON stations(name_normalized);
CREATE INDEX idx_country ON stations(countrycode);
CREATE INDEX idx_tags ON stations(tags);
CREATE INDEX idx_quality ON stations(quality_score);
CREATE INDEX idx_active ON stations(is_active);
CREATE INDEX idx_source ON stations(source);

-- Full-text search
CREATE VIRTUAL TABLE stations_fts USING fts5(
    name, description, tags, country,
    content=stations,
    content_rowid=rowid
);
...
CREATE INDEX idx_health_station ON health_history(station_id, checked_at);
```
...
- [ ] IPTV M3U parser
- [ ] Radio Garden full sync
- [ ] Playlist file collection
...
| Metric | Current | Goal |
|--------|---------|------|
| Total stations | 51K | 100K+ |
| Unique sources | 4 | 10+ |
| Active (verified) | ~45K | 90%+ |
| Avg quality score | ? | 70+ |
| Metadata completeness | ~60% | 90%+ |
| Daily health checks | 1x | Tiered |
| Uptime of service | ? | 99.9% |
