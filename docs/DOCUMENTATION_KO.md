...
Airtune은 55,000개 이상의 검증된 방송국을 지원하는 인터넷 라디오 플랫폼입니다. 두 가지 구성 요소로 이루어져 있습니다:
...
|-- radio_stations.db        # SQLite 데이터베이스 (51k+ 방송)
...
2. **Whisper** - DJ 멘트에서 음성-텍스트 변환 (openai-whisper 필요)
...
- openai-whisper (`pip install openai-whisper`) - DJ 음성 인식
